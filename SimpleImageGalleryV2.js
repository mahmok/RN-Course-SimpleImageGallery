...
export default class SimpleImageGalleryV2 extends React.Component {
    ...
    renderImagesWithTitles()
    {
        ...
        for(let i = 0; i < this.images.length; i++)
        {
            imagesComponents.push(
                <View key={"images-" + i} style={{flex: 1, flexDirection: 'row'}}>
                    <Image style={{flex: 0.8, height: 200}} source={{uri: this.images[i].uri}} resizeMode="contain" />
                    <Text style={{flex: 0.2, alignSelf: 'center', marginLeft: 5}}>{this.images[i].title}</Text>
                </View>
            );
        }
